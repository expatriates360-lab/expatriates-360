...
import {
  GraduationCap, Wrench, Award, Factory, Target, BookOpen, HandHeart, TrendingUp,
  ArrowUpRight,
} from "lucide-react";
 ...
 * and redirects users to Hunared.org.
 */

const PROGRAM_SERVICES = [
  { icon: GraduationCap, label: "Education Programs" },
  { icon: Wrench, label: "Technical Training" },
  { icon: Award, label: "International Certifications" },
  { icon: Factory, label: "Industrial Certifications" },
  { icon: Target, label: "Internships" },
  { icon: BookOpen, label: "Scholarships" },
  { icon: HandHeart, label: "Sponsorship Programs" },
  { icon: TrendingUp, label: "Career Development Programs" },
];
    ...
    >
      <div className="max-w-7xl mx-auto space-y-10">
        <div className="text-center max-w-2xl mx-auto">
          ...
          <h2 id="hunared-program-heading" className="text-3xl font-bold">
            Grow Your Skills with Hunared Organization
          </h2>
          <p className="text-muted-foreground mt-3">
            Education, training, certifications, and career development programs —
            delivered by Hunared Organization to help you build a stronger future.
          </p>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {PROGRAM_SERVICES.map(({ icon: Icon, label }) => (
            <a
              key={label}
              href={HUNARED_ORG_URL}
              target="_blank"
              rel="noopener noreferrer"
              className="group flex flex-col items-center gap-3 p-5 rounded-2xl border border-border bg-card text-center hover:border-primary/40 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200"
            >
